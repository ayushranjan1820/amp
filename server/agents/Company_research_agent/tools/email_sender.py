...
def send_company_research_email(
    recipients: List[str],
    subject: str,
    html_content: str,
    from_name: str = "ET-Labs Company Research"
) -> dict:
...
def format_company_report_as_html(
    company_name: str,
    report_html: str
) -> str:
    html = f"""
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #374151; max-width: 800px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #2c5f8a 100%); padding: 30px; border-radius: 12px 12px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 24px;">Company Research Report</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0; font-size: 16px;">{company_name}</p>
            <p style="color: rgba(255,255,255,0.7); margin: 5px 0 0 0; font-size: 12px;">Generated on {datetime.now().strftime('%B %d, %Y')}</p>
        </div>
