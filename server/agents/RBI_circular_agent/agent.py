...
from .tools.email_sender import send_rbi_email, format_rbi_report_as_html
from .ai_service import ai_service


class RBICircularAgent:
    """
    Context-aware RBI Circular Agent using Perplexity for web search
    ...
    def _search_tool_name() -> str:
        provider = RBICircularAgent._search_provider()
    ...
    async def send_email_report(
        ...
        """
        Send RBI circular report via email with AI-formatted content.
        ...
        email_formatting_prompt = f"""You are formatting an RBI circular research report for a professional email. Convert the following content into a well-structured, professional email format.
...
**Formatted Email Content:**"""
        ...
        try:
            ...
            formatted_content = f"<h3>RBI Circular Research Report</h3><p>{summary[:500]}...</p>"
        
        # Generate custom subject if not provided
        if not subject:
            try:
                subject_prompt = f"""Generate a professional, concise email subject line for this RBI circular report.
...
- Clearly indicates it's about RBI regulations/circulars
...
**Subject:**"""
                ...
                if not subject or len(subject) > 80:
                    subject = f"RBI Circular Report: {query[:40]}..."
            except:
                subject = f"RBI Circular Report: {query[:50]}..."
        ...
        html_content = format_rbi_report_as_html(query, formatted_content, sources)
        
        # Send email
        result = send_rbi_email(
    ...
    async def _analyze_query_intent(self, query: str, has_history: bool, last_context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        prompt = f"""You are an intelligent query router for an RBI information agent. Analyze whether the current query needs a NEW web search or can use PREVIOUS context.
...
- Query asks about a DIFFERENT RBI topic
...
**Response:**"""
        ...
        try:
            ...
            if 'USE_CONTEXT' in decision_clean or 'CONTEXT' in decision_clean:
                ...
                return {
                    'action': 'search',
                    'reason': 'AI determined query requires new RBI search',
                    'context_needed': False
                }
            else:
...
**Response:**"""
    ...
    def _filter_relevant_results(self, query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Programmatically filter search results to remove clearly irrelevant ones.
        Uses entity-type matching: if user asks about NBFCs, remove results clearly
        targeted only at banks/payment banks/SFBs etc.
        """
        if not results:
            return results
        ...
        entity_keywords = {
            'nbfc': ['nbfc', 'non-banking', 'non banking', 'housing finance', 'hfc', 'microfinance', 'mfi'],
            'bank': ['scheduled commercial bank', 'scb', 'commercial bank'],
            'payment_bank': ['payment bank', 'payments bank'],
            'sfb': ['small finance bank'],
            'cooperative': ['cooperative bank', 'co-operative bank', 'urban cooperative'],
        }
        ...
        target_keywords = entity_keywords.get(query_entity, [])
        
        exclude_entities = {k: v for k, v in entity_keywords.items() if k != query_entity}
        ...
        for r in results:
            ...
            is_relevant = any(kw in combined for kw in target_keywords)
            
            is_generic = any(kw in combined for kw in ['master direction', 'master circular', 'notification', 'guideline', 'regulation', 'framework'])
            
            is_exclusively_other = False
            if not is_relevant:
                for other_type, other_keywords in exclude_entities.items():
                    if any(kw in combined for kw in other_keywords):
                        if not any(kw in combined for kw in target_keywords):
                            is_exclusively_other = True
                            break
            
            if is_relevant or (is_generic and not is_exclusively_other) or (not is_exclusively_other):
                filtered.append(r)
            else:
                excluded_count += 1
                print(f"  ❌ Filtered out (irrelevant to {query_entity}): {r.get('title', 'N/A')[:80]}")
        
        if excluded_count:
            print(f"📋 Relevance filter: kept {len(filtered)}/{len(results)} results ({excluded_count} removed as irrelevant to {query_entity})")
    ...
    async def process_query(self, query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        3. Dynamically decide: search RBI OR use context OR hybrid
        ...
        if not self._provider_configured():
            if provider == "free_ollama":
                missing = "RBI_WEB_SEARCH_PROVIDER=free_ollama but OLLAMA_API_KEY is missing"
            elif provider == "free_duckduckgo":
                missing = "RBI_WEB_SEARCH_PROVIDER=free_duckduckgo but ddgs package is not installed"
            else:
                missing = "RBI_WEB_SEARCH_PROVIDER=perplexity but PERPLEXITY_API_KEY is missing"
            return self._format_error_response(query, missing, session_id, thinking_steps)

        thinking_steps.append({
            "type": "thinking",
            "content": "🔎 Initiating RBI website search"
        })
        
        # Step 1: Search RBI website using Perplexity
        thinking_steps.append({
            "type": "tool_call",
            "content": f"Searching RBI website via {provider_label} for relevant documents",
            "tool_name": search_tool_name,
            "tool_input": f"Query: {query}, Domain: https://www.rbi.org.in/"
        })
        
        try:
            search_results = await self.search.search_rbi(query, max_results=15)
        except Exception as e:
            thinking_steps.append({
                "type": "tool_result",
        ...
        search_results = self._filter_relevant_results(query, search_results)
        
        thinking_steps.append({
            "type": "tool_result",
            "content": f"✅ Found {len(search_results)} relevant documents from RBI website",
            "tool_name": search_tool_name
        })
        ...
        thinking_steps.append({
            ...
            "tool_input": f"Processing {len(search_results)} RBI documents"
        })
        ...
        thinking_steps.append({
            "type": "tool_result",
            "content": "✅ Successfully analyzed and formatted RBI documents",
            "tool_name": "pwc_llm_analyzer"
        })
        ...
        if email_recipients:
            ...
            thinking_steps.append({
                "type": "tool_call",
                "content": f"Sending RBI report via email to {len(email_recipients)} recipient(s)",
    ...
    def _basic_reformat(self, content: str, query: str) -> str:
        ...
        if any(kw in query_lower for kw in ['email', 'emai', 'emal', 'mail']):
            return f"""Subject: RBI Circular Information
...
"""
    ...
    def _create_basic_summary_from_perplexity(self, search_results: List[Dict[str, Any]], query: str) -> str:
        ...
        summary_parts.append(f"Found {len(search_results)} relevant documents from RBI website.\n\n")
    ...
    def _format_response(self, query: str, search_results: List[Dict[str, Any]], summary: str) -> str:
        ...
        response_parts.append("# 🏦 RBI Circular Analysis Report\n")
        response_parts.append(f"**Query:** {query}\n")
        response_parts.append(f"**Source:** Reserve Bank of India Official Website\n")
        ...
        response_parts.append("*All information is sourced from the official RBI website (https://www.rbi.org.in/). ")
        ...
        response_parts.append("🔗 [Visit RBI Website](https://www.rbi.org.in/) | ")
        response_parts.append("[Search RBI Circulars](https://www.rbi.org.in/scripts/NotificationUser.aspx)\n")
        
        return "".join(response_parts)
    
    def _format_no_results(self, query: str) -> str:
        """Format response when no results are found."""
        return f"""# 🏦 RBI Circular Search
...
No relevant RBI circulars or notifications were found for your query.

### 💡 Suggestions:
- Try using different keywords or phrases
- Use specific terms like:
  - "KYC" instead of "customer verification"
  - "NBFC" instead of "non-banking finance company"
  - Specific circular numbers if known (e.g., "RBI/2023-24/123")
- Search for broader topics first, then narrow down
- Check for official terminology used by RBI

### 🔗 Alternative Resources:
- [Browse All RBI Notifications](https://www.rbi.org.in/scripts/NotificationUser.aspx)
- [RBI Master Circulars](https://www.rbi.org.in/scripts/BS_ViewMasCirculardetails.aspx)
- [RBI FAQs](https://www.rbi.org.in/Scripts/FAQView.aspx)
- [RBI Homepage](https://www.rbi.org.in/)

---
*Try refining your search or explore the RBI website directly using the links above.*
"""
...
## Error Details
An error occurred while searching the RBI website:
...
3. **Visit RBI directly** - [RBI Website](https://www.rbi.org.in/)
