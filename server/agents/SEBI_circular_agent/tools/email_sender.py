...
def send_sebi_email(
    ...
    subject: str,
    html_content: str,
    from_name: str = "ET-Labs SEBI Circular Agent"
) -> dict:
    """
    Send the SEBI circular research report via email using Resend.
    ...
    try:
        ...
        try:
            ...
            log_cost_event(event_type="email_send", agent_name="SEBI Circular Agent", metadata={"recipients": recipients, "subject": subject})
        except Exception:
            pass
...
def format_sebi_report_as_html(
    query: str,
    summary: str,
    sources: List[dict]
) -> str:
    """
    Format the SEBI circular report as professional HTML email.
    
    Args:
        query: The user's query
        summary: The formatted summary/response from the agent
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #374151; max-width: 800px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #D85604 0%, #E88D14 100%); padding: 30px; border-radius: 12px 12px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 24px;">SEBI Circular Research Report</h1>
